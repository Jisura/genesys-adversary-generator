...
function update() {
    ...
    result = {
        "power_level": {"combat": 0, "social": 0, "general": 0},
    ...
    for (var i = 0, n = choices.length; i < n; i++) {
        for (var item in choices[i]) {
            var values = choices[i][item];

            if (item == 'caracs') {
                result['caracteristics'] = values;
                result['defense']['soak'] += values['brawn']; // Add brawn to soak
            } else if (item == 'power_level') {
                for (var name in values)
                    result["power_level"][name] += values[name];
            } else if (item == 'skills') {
                for (var name in values) {
                    var prev = (name in result["skills"]) ? result["skills"][name] : 0;
                ...
                result["equipment"].push(values); // Store equipment details
                if (values.soakModifier) {
                    result['defense']['soak'] += values.soakModifier; // Add equipment soak modifier
                }
                if (values.defenseModifier) {
                    result['defense']['melee_defense'] += values.defenseModifier.melee || 0;
                    result['defense']['ranged_defense'] += values.defenseModifier.ranged || 0;
                }
    ...
    // Adjust wound threshold based on adversary type
    ...
    text.innerHTML = abilities.sort().join(', ') + '.';
    ...
    if ('equipment' in result && result.equipment.length > 0) 
        text.textContent = result['equipment'].map(eq => eq.description || eq).join(', ');
    else
        text.innerHTML = 'None.';
